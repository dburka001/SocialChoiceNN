...
class mlp:
    ...
    def earlystopping(self,inputs,targets,valid,validtargets,eta,niterations=100):
        ...
        # BD: Added abs, cos in some cases val_error was increasing
        while ((((old_val_error1 - new_val_error) > 0.001) or ((old_val_error2 - old_val_error1) > 0.001) or ((old_val_error1 - new_val_error) < 0) or ((old_val_error2 - old_val_error1) < 0)) and count <= 1000):
            count+=1
            # print count
    ...
    def confmat(self,inputs,targets):
        ...
        print
        print "\nConfusion matrix of MLP is:"
        print cm
        print "Percentage Correct: ",trace(cm)/sum(cm)*100
        self.correct = trace(cm)/sum(cm)*100
        return trace(cm)/sum(cm)*100

    def get_data(self, resultPath, filename, seed):
        import csv
        with open(resultPath + filename + '.' + str(seed) + '.network', 'wb') as csvfile:
            writer = csv.writer(csvfile, delimiter=';')
            writer.writerow(['Hidden:'] + [self.nhidden])                    
            writer.writerow(['Correct:'] + [self.correct])
            writer.writerow(['Inputs:'] + [self.nin])
            writer.writerow(['Outputs:'] + [self.nout])
            writer.writerow([])
            writer.writerow(['beta'])
            writer.writerow([self.beta])
            writer.writerow([])
            writer.writerow(['weights1'])
            for line in self.weights1:
                writer.writerow(line)
            writer.writerow([])
            writer.writerow(['weights2'])    
            for line in self.weights2:
                writer.writerow(line)
